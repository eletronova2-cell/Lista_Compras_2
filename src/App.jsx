...
function TelaHome({ itens, historico, temRascunho, selecionados, onIrListaBase, onIrPreparar, onIrComprar, onIrHistorico, onIrLocalizacao }) {
  ...
  return (
    <div style={BASE}>
      ...
      <div style={{ textAlign:"center", fontSize:11, color:"rgba(255,255,255,0.15)", fontWeight:600, paddingBottom:16 }}>
        v {APP_VERSION}
      </div>
    </div>
  );
...
function TelaSupermercados({ itens, selecionados, onEscolherSupermercado, onVoltar }) {
  const [fase, setFase]             = useState("localizando");
  ...
  // ── Overpass API — supermercados reais num raio de 5km ──────────────────────
  const buscarOverpass = async (lat, lng) => {
    const query = `[out:json][timeout:15];(node["shop"="supermarket"](around:5000,${lat},${lng});way["shop"="supermarket"](around:5000,${lat},${lng}););out center 20;`;
    const resp = await fetch("https://overpass-api.de/api/interpreter", {
      method:"POST", body:"data="+encodeURIComponent(query)
    });
    if (!resp.ok) throw new Error("Overpass falhou");
    const data = await resp.json();
    const comDist = (data.elements||[]).map(el => {
      const elLat = el.lat ?? el.center?.lat;
      const elLng = el.lon  ?? el.center?.lon;
      const nome  = (el.tags||{}).name || (el.tags||{}).brand || "";
      if (!nome || !elLat) return null;
      const dLat = (elLat-lat)*Math.PI/180, dLng=(elLng-lng)*Math.PI/180;
      const a = Math.sin(dLat/2)**2 + Math.cos(lat*Math.PI/180)*Math.cos(elLat*Math.PI/180)*Math.sin(dLng/2)**2;
      return { nome, distM:Math.round(6371000*2*Math.asin(Math.sqrt(a))), lat:elLat, lng:elLng };
    }).filter(Boolean).sort((a,b)=>a.distM-b.distM);
    const vistos=new Set(), unicos=[];
    for(const s of comDist){ const k=s.nome.toLowerCase().replace(/\s+/g,""); if(!vistos.has(k)){vistos.add(k);unicos.push(s);} }
    return unicos.slice(0,15);
  };

  // ── GPS automático ao montar ────────────────────────────────────────────────
  useEffect(() => {
    if (!navigator.geolocation) { setFase("manual"); return; }
    navigator.geolocation.getCurrentPosition(
      async pos => {
        ...
        fetch(`https://nominatim.openstreetmap.org/reverse?lat=${lat}&lon=${lng}&format=json`)
          .then(r=>r.json())
          .then(d=>{ const c=d.address?.city||d.address?.town||d.address?.village||""; const e=d.address?.state_code||""; setLocalLabel(c?`${c}${e?", "+e:""}`:`${lat.toFixed(3)}, ${lng.toFixed(3)}`); })
          .catch(()=>setLocalLabel(`${lat.toFixed(3)}, ${lng.toFixed(3)}`));
        try {
          const lista = await buscarOverpass(lat, lng);
          if (lista.length===0) { setErroBusca("Nenhum supermercado encontrado num raio de 5km."); setFase("manual"); }
          else { setSupermercados(lista); setFase("resultado"); }
        } catch { setErroBusca("Não foi possível buscar supermercados. Digite sua cidade."); setFase("manual"); }
      },
      () => setFase("manual"),
  ...
  const buscarPorTexto = async () => {
    ...
    setFase("localizando"); setErroBusca("");
    try {
      const geo = await fetch(`https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(cidadeManual)}&format=json&limit=1&countrycodes=br`).then(r=>r.json());
      if (!geo.length) throw new Error("Cidade não encontrada");
      const lat=parseFloat(geo[0].lat), lng=parseFloat(geo[0].lon);
      setCoords({ lat, lng });
      setLocalLabel(geo[0].display_name.split(",").slice(0,2).join(",").trim());
      const lista = await buscarOverpass(lat, lng);
      if (lista.length===0) { setErroBusca(`Nenhum supermercado encontrado em "${cidadeManual}".`); setFase("manual"); }
      else { setSupermercados(lista); setFase("resultado"); }
    } catch(e) { setErroBusca(e.message||"Erro ao buscar. Tente novamente."); setFase("manual"); }
  };
  ...
  if (fase==="localizando") return (
    <div style={{ ...BASE_STYLE, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:24 }}>
      ...
      <div style={{ fontWeight:800, fontSize:20, marginBottom:8, textAlign:"center" }}>Buscando supermercados…</div>
      <div style={{ fontSize:14, color:"rgba(255,255,255,0.4)", textAlign:"center", animation:"pulse 2s ease-in-out infinite", marginBottom:32 }}>Localizando opções na sua região</div>
      <button onClick={()=>setFase("manual")} style={{ background:"rgba(255,255,255,0.06)", border:"1px solid rgba(255,255,255,0.12)", borderRadius:12, padding:"12px 24px", color:"rgba(255,255,255,0.5)", fontSize:13, fontWeight:700, cursor:"pointer", fontFamily:"'Plus Jakarta Sans',sans-serif" }}>
        Pular localização
      </button>
    </div>
  );
    ...
    <div style={BASE_STYLE}>
      ...
      <div style={{ background:"rgba(255,255,255,0.04)", backdropFilter:"blur(20px)", borderBottom:"1px solid rgba(255,255,255,0.08)", padding:"14px 20px" }}>
        <div style={{ maxWidth:620, margin:"0 auto", display:"flex", alignItems:"center", gap:12 }}>
          ...
          <span style={{ fontWeight:800, fontSize:18 }}>📍 Informe sua localização</span>
        </div>
      </div>
      <div style={{ maxWidth:520, margin:"0 auto", padding:"32px 20px" }}>
        {erroBusca&&<div style={{ background:"rgba(251,191,36,0.08)", border:"1px solid rgba(251,191,36,0.25)", borderRadius:12, padding:"10px 14px", marginBottom:20, fontSize:13, color:"#fbbf24" }}>⚠️ {erroBusca}</div>}
        <div style={{ fontSize:13, color:"rgba(255,255,255,0.5)", marginBottom:12 }}>Digite o nome da sua cidade ou bairro:</div>
        <div style={{ display:"flex", gap:8, marginBottom:16 }}>
          <input type="text" value={cidadeManual} onChange={e=>setCidadeManual(e.target.value)}
            onKeyDown={e=>e.key==="Enter"&&buscarPorTexto()} placeholder="ex: Moema, São Paulo" autoFocus
          ...
          <button onClick={buscarPorTexto} disabled={!cidadeManual.trim()}
            style={{ background:cidadeManual.trim()?"linear-gradient(135deg,#6366f1,#8B5CF6)":"rgba(255,255,255,0.05)", border:"none", borderRadius:12, padding:"14px 20px", color:cidadeManual.trim()?"#fff":"rgba(255,255,255,0.2)", fontWeight:900, fontSize:18, cursor:cidadeManual.trim()?"pointer":"not-allowed" }}>→</button>
        </div>
        <button onClick={()=>onEscolherSupermercado("")}
          style={{ width:"100%", background:"rgba(16,185,129,0.1)", border:"1.5px solid rgba(16,185,129,0.3)", borderRadius:14, padding:"14px", color:"#34d399", fontWeight:800, fontSize:14, cursor:"pointer", fontFamily:"'Plus Jakarta Sans',sans-serif" }}>
          🛒 Ir comprar sem escolher mercado
        </button>
      </div>
    </div>
  );
    ...
    <div style={BASE_STYLE}>
      ...
      <div style={{ background:"rgba(255,255,255,0.04)", backdropFilter:"blur(20px)", borderBottom:"1px solid rgba(255,255,255,0.08)", padding:"14px 20px", position:"sticky", top:0, zIndex:100 }}>
        <div style={{ maxWidth:660, margin:"0 auto", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
          <div style={{ display:"flex", alignItems:"center", gap:12 }}>
            <button onClick={onVoltar} style={{ background:"rgba(255,255,255,0.07)", border:"1px solid rgba(255,255,255,0.1)", borderRadius:10, padding:"8px 14px", color:"rgba(255,255,255,0.6)", fontSize:13, fontWeight:700, cursor:"pointer" }}>← Voltar</button>
            <div>
              <div style={{ fontWeight:800, fontSize:18 }}>🏪 Supermercados próximos</div>
              <div style={{ fontSize:11, color:"rgba(255,255,255,0.4)", marginTop:1 }}>📍 {localLabel||"sua região"} · raio 5km</div>
            </div>
          </div>
          {coords&&<button onClick={()=>window.open(`https://www.google.com/maps/search/supermercado/@${coords.lat},${coords.lng},14z`,"_blank")} style={{ background:"rgba(99,102,241,0.15)", border:"1px solid rgba(99,102,241,0.3)", borderRadius:10, padding:"8px 12px", color:"#a5b4fc", fontSize:12, fontWeight:700, cursor:"pointer" }}>🗺️ Maps</button>}
        </div>
      </div>
      <div style={{ maxWidth:660, margin:"0 auto", padding:"16px 16px" }}>
        <div style={{ background:"rgba(255,255,255,0.04)", border:"1px solid rgba(255,255,255,0.08)", borderRadius:14, padding:"10px 16px", marginBottom:14, display:"flex", justifyContent:"space-between", alignItems:"center" }}>
          <div style={{ fontSize:13, color:"rgba(255,255,255,0.6)" }}>📋 <strong style={{ color:"#fff" }}>{itensCompra.length} itens</strong>{totalEstimado>0?` · Est. R$ ${totalEstimado.toFixed(2)}`:""}</div>
          <div style={{ fontSize:11, color:"rgba(255,255,255,0.35)" }}>{supermercados.length} encontrados</div>
        </div>
        ...
        {supermercados.map((s,i)=>(
          <div key={i} onClick={()=>onEscolherSupermercado(s.nome)}
            ...
            <div style={{ width:42, height:42, borderRadius:12, background:"rgba(99,102,241,0.15)", border:"1.5px solid rgba(99,102,241,0.25)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:22, flexShrink:0 }}>🏪</div>
            <div style={{ flex:1, minWidth:0 }}>
              <div style={{ fontWeight:800, fontSize:14, color:"#fff", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{s.nome}</div>
              <div style={{ fontSize:11, color:"rgba(255,255,255,0.35)", marginTop:2 }}>📍 {fmt(s.distM)} de distância</div>
            </div>
            <div style={{ display:"flex", gap:8, flexShrink:0 }}>
              {coords&&<button onClick={e=>{e.stopPropagation();window.open(`https://www.google.com/maps/dir/?api=1&origin=${coords.lat},${coords.lng}&destination=${s.lat},${s.lng}&travelmode=driving`,"_blank");}} style={{ background:"rgba(99,102,241,0.12)", border:"1px solid rgba(99,102,241,0.25)", borderRadius:8, padding:"6px 10px", color:"#a5b4fc", fontSize:11, fontWeight:700, cursor:"pointer" }}>📍 Rota</button>}
              <div style={{ fontSize:18, color:"rgba(16,185,129,0.6)", display:"flex", alignItems:"center" }}>→</div>
            </div>
          </div>
        ))}
        <div style={{ marginTop:16, marginBottom:8 }}>
          <div style={{ fontSize:11, color:"rgba(255,255,255,0.3)", fontWeight:700, marginBottom:8 }}>Não está na lista? Digite o nome:</div>
        ...
        <button onClick={()=>{setCidadeManual("");setErroBusca("");setFase("manual");}}
